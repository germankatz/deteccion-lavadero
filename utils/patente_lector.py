...
def leer_patente(img_patente,roi2):
    ...
    alto, ancho = img_patente.shape[:2]
    roi = (0, 0, ancho, alto)
    img = rectificar_roi(img_patente,roi,25)
    #cv2.imshow("Patente rotada", img)
    #cv2.waitKey(0)    

    x, y, w, h = roi2
    img_recrotada = img[y:y+h, x:x+w]
    #cv2.imshow("Patente recortada", img_recrotada)
    #cv2.waitKey(0)
    ...
    #izq, centro, der = dividir_patente_en_tres(img_recrotada)
    ...
    patente_final = pytesseract.image_to_string(img_recrotada, config=config).strip()
...
def dividir_patente_en_tres(img_patente):
    ...
    centro    = img_patente[:, tercio:3*tercio]
    cv2.imshow("centro", centro)
    cv2.waitKey(0)  # Espera que presiones una tecla
    cv2.destroyAllWindows()
    derecha   = img_patente[:, 3*tercio:]
...
def extraer_patente_desde_roi(imagen, roi):
    """
    Recorta y rectifica una región (ROI) de la imagen.
    
    Parámetros:
    - imagen: imagen de entrada (BGR)
    - roi: tupla (x, y, w, h) que define el rectángulo de interés

    Retorna:
    - imagen rectificada del ROI
    """
    x, y, w, h = roi

    # Definimos las esquinas del ROI original (en la imagen de entrada)
    pts = np.array([
    ...
    # Coordenadas destino (rectángulo rectificado)
    dst_pts = np.array([
        [0, 0],
        [w - 1, 0],
        [w - 1, h - 1],
    ...
    # Obtener matriz de transformación perspectiva
    M = cv2.getPerspectiveTransform(pts, dst_pts)

    # Aplicar la transformación
    warp = cv2.warpPerspective(imagen, M, (w, h))
...
#base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # sube desde utils/
#ruta_img = os.path.join(base_dir, 'img_src', 'imgRotada.png')
...
#img_rectificada = extraer_patente_desde_rectangulo_verde(img)
#x, y, w, h = roi

#texto = leer_patente(img_rectificada, roi)
